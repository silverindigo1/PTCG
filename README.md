...
The whole stack runs in a Claude Code cloud session, which you start from the
**Code** tab in the Claude app. Those sessions ship with Python, pytest, Docker
and PostgreSQL 16, and start PostgreSQL only when asked
(https://code.claude.com/docs/en/cloud-environments).

`.claude/settings.json` registers `scripts/cloud_session_start.sh` as a
SessionStart hook. In a cloud session it installs the Python dependencies,
starts PostgreSQL, creates the `pokearb` role and database, and applies the
migrations and seeds on first use. On a laptop it exits immediately. It is
idempotent, so resuming a session costs about a second. The Makefile defaults
`DATABASE_URL` to the database the hook creates, so in a cloud session the
whole instruction is:

```
Run make test and make demo and report the results.
```

Python 3.11 or later. The suite has been run under 3.11.15 and 3.12.
