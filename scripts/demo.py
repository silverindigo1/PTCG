...
What this proves, against a real PostgreSQL database and with no gated API:
  ...
  7. An alert fires on a first sighting and then only on a material change.
...
def main() -> int:
    ...
    # Every query below is scoped to this demo's own card, so rows written by
    # anything else in the same database, such as the test suite, cannot
    # appear in the demo's output.
    with store.conn.cursor() as cur:
        cur.execute(
            "SELECT opportunity_id FROM opportunity WHERE variant_id = %s::uuid",
            (variant_id,),
        )
        row = cur.fetchone()
        opportunity_id = str(row["opportunity_id"]) if row else None
        cur.execute(
            """
    ...
    banner("6. Alerts: three cycles ran, an alert needs a material change")
    # Printed row by row with the reason each alert gave, rather than as a bare
    # count, so the output shows the rule working instead of asserting it.
    with store.conn.cursor() as cur:
        cur.execute(
            "SELECT sent_at, payload FROM alert WHERE opportunity_id = %s::uuid "
            "ORDER BY sent_at",
            (opportunity_id,),
        )
        rows = cur.fetchall()
    print(f"  cycles run: 3, alerts raised: {len(rows)}")
    for row in rows:
        reasons = "; ".join(row["payload"].get("reasons", []))
        print(f"  {row['sent_at']:%H:%M:%S}  {reasons}")
    print("  re-importing the same file, and adding one sale, are not material changes")

    banner("7. Production data is untouched by the demo")
    with store.conn.cursor() as cur:
        cur.execute(
            "SELECT dataset, count(*) AS n FROM sale WHERE variant_id = %s::uuid "
            "GROUP BY dataset",
            (variant_id,),
        )
        for row in cur.fetchall():
            print(f"  demo card, {row['dataset']}: {row['n']} sale row(s)")
        cur.execute("SELECT count(*) AS n FROM sale WHERE dataset = 'production'")
        print(f"  production sales in the whole database: {cur.fetchone()['n']}")
